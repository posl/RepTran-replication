...
def main(ds_name, k, tgt_rank, n, misclf_type, fpfn, run_all=True, fl_methods=None, agg="abs_mean"):
    ...
    if fl_methods is None:
        for op in ["enh", "sup"]:
            for fl_method in ["vdiff", "random"]:
                ...
                if agg == "abs_mean":
                    ...
                    diff_proba_mean[(op, fl_method)] = np.mean(diff_proba[(op, fl_method)])
                else:
        ...
        fl_method = "_".join(fl_methods) # len(fl_methods)が1のときはfl_methods[0]そのまま
        proba_save_dir = os.path.join(save_dir, f"proba_n{n}_{fl_method}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agg", type=str, help="aggregate results", default="abs_mean")
    ...
    if run_all:
        ...
        for k, tgt_rank, n, misclf_type, fpfn in tqdm(product(k_list, tgt_rank_list, n_list, misclf_type_list, fpfn_list), total=len(k_list)*len(tgt_rank_list)*len(n_list)*len(misclf_type_list)*len(fpfn_list)):
            ...
            result_df = main(ds, k, tgt_rank, n, misclf_type, fpfn, run_all=run_all, agg=agg)
            all_results = pd.concat([all_results, result_df], ignore_index=True)
        # all_resultsを保存
        all_results.to_csv(f"./{ds}_proba_diff_{agg}_run_all.csv", index=False)
    elif run_asc_desc:
        true_labels = range(100) if ds == "c100" else None
        tgt_split = "repair"
        k_list = range(5)
        ...
        for k, tgt_rank, n, misclf_type, fpfn in tqdm(product(k_list, tgt_rank_list, n_list, misclf_type_list, fpfn_list), total=len(k_list)*len(tgt_rank_list)*len(n_list)*len(misclf_type_list)*len(fpfn_list)):
            if (misclf_type == "src_tgt" or misclf_type == "all") and fpfn is not None:
                continue
            # 各パラメータの組み合わせでmain関数から1行分のDataFrameを取得し、all_resultsに連結
            result_df = main(ds, k, tgt_rank, n, misclf_type, fpfn, run_all=run_all, fl_methods=fl_methods, agg=agg)
            all_results = pd.concat([all_results, result_df], ignore_index=True)
        # all_resultsを保存
        all_results.to_csv(f"./{ds}_proba_diff_{agg}_run_asc_desc_{'_'.join(fl_methods)}.csv", index=False)
    else:
