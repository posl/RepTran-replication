...
def gen_and_save_random_location(wnum, model, save_path):
    ...
    # Split into before/after and restore original shape
...
def delete_old_files(patch_save_path, tracker_save_path, tgt_indices_save_path, metrics_json_path, wnum):
    # Iterate through arguments together
    for path in (patch_save_path, tracker_save_path, tgt_indices_save_path, metrics_json_path):
        dirname  = os.path.dirname(path)          # Directory
        basename = os.path.basename(path)         # Filename
...
if __name__ == "__main__":
    ...
    # ==================================================================
    patch_save_path = os.path.join(save_dir, f"exp-repair-5-1-best_patch_{setting_id}_{fl_method}_reps{reps_id}.npy")
    tracker_save_path = os.path.join(save_dir, f"exp-repair-5-1-tracker_{setting_id}_{fl_method}_reps{reps_id}.pkl")
    # Save tgt_indices of data used for repair as npy
    tgt_indices_save_path = os.path.join(save_dir, f"exp-repair-5-1-tgt_indices_{setting_id}_{fl_method}_reps{reps_id}.npy") # TODO: If randomness is involved in determining tgt_indices, need to make filename that can track it. Even for same setting_id, fl_method but different repetition
    metrics_json_path = os.path.join(save_dir, f"exp-repair-5-1-metrics_for_repair_{setting_id}_{fl_method}_reps{reps_id}.json") # Record execution time for each setting
