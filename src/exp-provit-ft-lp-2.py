"""Launcher: run PRoViTFT+LP for reps 0..n_reps-1, each in its own subprocess
(exp-provit-ft-lp-1.py). Isolates segfaults (a crash kills only that rep) and
releases GPU memory between reps. A crashed rep is retried; the runner saves per
benchmark and resumes, so a retry continues where it left off. After all reps,
the per-rep JSONs are merged into a single combined results file.

Usage:
    python exp-provit-ft-lp-2.py c100 0
    python exp-provit-ft-lp-2.py tiny-imagenet 0 --n-reps 5
"""
...
if __name__ == "__main__":
    ...
    out = os.path.join(save_dir, f"results_lr{args.lr}_eps{args.eps}.json")
