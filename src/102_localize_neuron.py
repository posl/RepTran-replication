...
def main(ds_name, k, tgt_rank, theta, fl_method, misclf_type, fpfn, run_all=False):
    print(f"ds_name: {ds_name}, fold_id: {k}, tgt_rank: {tgt_rank}, theta: {theta}, fl_method: {fl_method}, misclf_type: {misclf_type}, fpfn: {fpfn}")
    
    # pretrained modelのディレクトリ
    pretrained_dir = getattr(ViTExperiment, ds_name).OUTPUT_DIR.format(k=k)
    # 結果とかログの保存先を先に作っておく
    save_dir = os.path.join(pretrained_dir, f"misclf_top{tgt_rank}", f"{misclf_type}_neurons_location")
    if misclf_type == "all":
        save_dir = os.path.join(pretrained_dir, f"all_neurons_location")
    if fpfn is not None and misclf_type == "tgt":
        save_dir = os.path.join(pretrained_dir, f"misclf_top{tgt_rank}", f"{misclf_type}_{fpfn}_neurons_location")
    location_save_path = os.path.join(save_dir, f"location_theta{theta}_{fl_method}.npy")
    ...
    exp_name = f"{this_file_name}_theta{theta}" if not run_all else f"{this_file_name}_run_all"
    # loggerの設定をして設定情報を表示
    logger = set_exp_logging(exp_dir=save_dir, exp_name=exp_name)
    logger.info(f"ds_name: {ds_name}, fold_id: {k}, tgt_rank: {tgt_rank}, theta: {theta}, fl_method: {fl_method}, misclf_type: {misclf_type}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('theta', nargs="?", type=int, help="the percentage for the number of neurons to fix (%)")
    ...
    if run_all:
        ...
        assert k_list is None and tgt_rank_list is None and theta_list is None, "run_all and k_list or tgt_rank_list or theta_list cannot be specified at the same time"
        k_list = range(5)
        tgt_rank_list = range(1, 6)
        theta_list = [5, 10, 15, 20, 25, 30]
        misclf_type_list = ["all", "src_tgt", "tgt"]
        fpfn_list = [None, "fp", "fn"]
        for k, tgt_rank, theta, misclf_type, fpfn in product(k_list, tgt_rank_list, theta_list, misclf_type_list, fpfn_list):
        ...
        assert k_list is not None and tgt_rank_list is not None and theta_list is not None, "k_list and tgt_rank_list and n_list should be specified"
