...
if __name__ == "__main__":
    ...
    for ds_name, ds, ls in zip(ds_name_list, ds_list, label_list):
        ...
        for tgt_label in range(num_labels):
            print(f"{'='*60}\ntgt_ds: {ds_name}, tgt_label: {tgt_label}\n{'='*60}")
            # labelsがtgt_labelのindexを取得
            tgt_idx = np.where(ls == tgt_label)[0]
            tgt_ds = ds.select(tgt_idx)
            ...
            is_correct = all_pred_labels == tgt_label
            ...
            for cor_mis, mid_states in zip(["cor", "mis"], [correct_mid_states, incorrect_mid_states]):
                ...
                vscore_save_path = os.path.join(result_dir, f"vscore_l{start_li}tol{end_li}_{tgt_label}_{ds_type}_{cor_mis}.npy")
