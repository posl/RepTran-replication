...
def get_location_path(n, w_num, fl_method, location_dir, generate_random=True):
    if fl_method == "ours":
        location_file = f"exp-fl-10_location_n{n}_weight_loss.npy"
    elif fl_method == "bl":
...
def get_loss_diff_path(n, w_num, fl_method, loss_diff_dir, op, cor_mis):
    ...
    if fl_method == "ours":
        loss_diff_file = f"exp-fl-10_loss_diff_n{n}_{op}_{cor_mis}_weight_loss.npy"
    elif fl_method == "bl":
    ...
    for tgt_rank in tgt_rank_list:
        for fl_method in fl_method_list:
            ...
            for exp in exp_list:
                ...
                print(f"========================")
                df_tmp = main(fl_method=fl_method, n=n_ratio, w_num=w_num, rank=tgt_rank)
                all_results.append(df_tmp)
    df_all = pd.concat(all_results, ignore_index=True)
    unique_ns = df_all["n_ratio"].unique()
    n_str = '_'.join(map(str, unique_ns))
    output_csv_all = os.path.join(os.getcwd(), f"exp-fl-10-2_n{n_str}.csv")
