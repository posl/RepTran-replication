...
def main(ds_name, k, tgt_rank, misclf_type, fpfn, w_num, beta=None):
    ...
    # Load dataset (true_labels are needed)
    ...
    tgt_split = "repair"  # NOTE: we only use the repair split for repairing
    ...
    if misclf_type == "tgt":
        ...
        # Select all correct samples even for tgt (merge correct-tgt and correct-others)
    ...
    cached_mid_states = torch.load(mid_save_path, map_location="cpu")  # (num_samples in repair split, num_neurons=3072)
    ...
    # Execute localization using v-score and mean activation
    ...
    # Next, identify weights also using gradient information.
    # ============================================================
    ...
    # Use vit_utils functions to get batches
    ...
    # Check shape consistency of hidden_states_before_layernorm
    ...
    # Load model required for gradient computation
    ...
    # Collect overall grad_loss and fwd_imp
    ...
    # From here: adjust grad_loss and fwd_imp with neuron_scores
    ...
    for i in range(2):
        ...
        else:  # this variant may be less effective
...
if __name__ == "__main__":
    ...
    for ds in ds_list:
        for k, tgt_rank, misclf_type, fpfn, w_num in product(k_list, tgt_rank_list, misclf_type_list, fpfn_list, w_num_list):
            ...
            if (misclf_type == "src_tgt" or misclf_type == "all") and fpfn is not None:  # for src_tgt or all, fpfn must be None
                continue
            if misclf_type == "tgt" and fpfn is None:  # for tgt, fpfn must be specified
                continue
