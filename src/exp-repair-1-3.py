...
def get_location_save_path(fl_method, n, wnum=None):
    """
    1-1 で使用するロジックと同様に、
    fl_method と n (必要なら wnum) を用いて locationファイル名を生成
    """
    ...
    if fl_method != "vmg":
        ...
        assert wnum is not None, "wnum should be specified for vmg."
...
if __name__ == "__main__":
    ...
    parser.add_argument("--custom_n", type=float, default=None)
    ...
    if args.setting_path is not None:
        ...
        setting_id = "default" if (
            custom_n is None and custom_alpha is None and 
            custom_bounds is None and custom_wnum is None
        ) else ""
        parts = []
    ...
    n_val = setting_dic["n"]
    wnum_val = setting_dic.get("wnum", None)
    location_filename = get_location_save_path(fl_method, n_val, wnum=wnum_val)
    ...
    patch_filename = f"exp-repair-1-best_patch_{setting_id}_{fl_method}_reps{reps_id}.npy"
    ...
    #==================================================
    exp_name = f"exp-repair-1-3_{setting_id}_{fl_method}"
