...
def main(ds_name, k, tgt_rank, misclf_type, fpfn, n, sample_from_correct=False, strategy="weighted"):
    ...
    places_to_neuron, tgt_neuron_score = localize_neurons_with_mean_activation(vscore_before_dir=None, vscore_dir=vscore_dir, vscore_after_dir=None, tgt_layer=tgt_layer, n=n, intermediate_states=intermediate_states, tgt_mis_indices=tgt_mis_indices, misclf_pair=misclf_pair, tgt_label=tgt_label, fpfn=fpfn, alpha=0.5)
    ...
    location_file_name = f"exp-fl-5_location_nAll_wAll_neuron.npy" if n is None else f"exp-fl-5_location_n{n}_w{w_num}_neuron.npy"
    ...
    print(f"tgt_neuron_score.shape: {tgt_neuron_score.shape}") # (num_tgt_neurons, )
    ...
    grad_Wbef_row = results["before"]  # shape: (3072, 768)
    grad_Waft_col = results["after"]  # shape: (768, 3072)
    print(f"grad_Wbef_row.shape: {grad_Wbef_row.shape}, grad_Waft_col.shape: {grad_Waft_col.shape}")

    # tgt_neuron_score同様に{grad_Wbef_row, grad_Waft_col}もmin-max scalerする
    combined_min = min(grad_Wbef_row.min(), grad_Waft_col.min())
    combined_max = max(grad_Wbef_row.max(), grad_Waft_col.max())
    # Min-Max スケーリングを適用
    grad_Wbef_row = (grad_Wbef_row - combined_min) / (combined_max - combined_min)
    grad_Waft_col = (grad_Waft_col - combined_min) / (combined_max - combined_min)
    
    # Wbef_{ji}はhbef_iとhmid_jをつなぐとする．このときhmid_jに対するニューロンスコアns_jとの重み付き和が大きいものが重要なニューロンとなる．
    # Waft_{ji}はhmid_iとhaft_jをつなぐとする．このときhmid_iに対するニューロンスコアns_iをの重み付き和が大きいものが重要なニューロンとなる．
    num_total_neuron = len(grad_Wbef_row)
    indexed_tgt_neuron_score = np.zeros(num_total_neuron)
    indexed_tgt_neuron_score[indices_to_neurons] = tgt_neuron_score
    wscore_bef = 0.5 * grad_Wbef_row + 0.5 * indexed_tgt_neuron_score[:, np.newaxis]  # shape: (num_tgt_neuron, 768)
    wscore_aft = 0.5 * grad_Waft_col + 0.5 * indexed_tgt_neuron_score # shape: (768, num_tgt_neuron)
    
    # フラット化
    wscore_bef_flat = wscore_bef.flatten()  # shape: (num_neurons * 768,)
    wscore_aft_flat = wscore_aft.flatten()  # shape: (768 * num_neurons,)

    # 結合
    combined = np.concatenate((wscore_bef_flat, wscore_aft_flat))  # shape: (num_neurons * 768 + 768 * num_neurons,)
    ...
    print(f"saved location information to {location_save_path}")
    rank_file_name = f"exp-fl-5_location_nAll_wAll_weight_rank.npy" if n is None else f"exp-fl-5_location_n{n}_w{w_num}_weight_rank.npy"
    rank_save_path = os.path.join(location_save_dir, rank_file_name)
...
if __name__ == "__main__":
    ...
    results = []
    n_list = [None, NUM_IDENTIFIED_NEURONS]
    # n_list = [None]
    for k, tgt_rank, misclf_type, fpfn, n in product(k_list, tgt_rank_list, misclf_type_list, fpfn_list, n_list):
        print(f"Start: ds={ds}, k={k}, tgt_rank={tgt_rank}, misclf_type={misclf_type}, fpfn={fpfn}, n={n}")
