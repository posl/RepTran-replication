...
if __name__ == "__main__":
    ...
    for tgt_ct in ct_list:
        ...
        for tgt_label in range(num_labels):
            ...
            # tmp_petal_labelsの各値をfloatの数値に直す
            tmp_petal_labels = {
        ...
        venn_result_path = os.path.join(venn_result_dir, f"./{tgt_ct}_{used_column}.pdf")
    ...
    venn_result_dir = os.path.join(result_dir, "venn")
    os.makedirs(venn_result_dir, exist_ok=True)
    venn_result_path = os.path.join(venn_result_dir, f"./all_{used_column}.pdf")
    plt.savefig(venn_result_path, dpi=300)
    print(f"Venn diagram is saved at {venn_result_path}")
