...
def main(ds_name, k, tgt_rank, misclf_type, fpfn, fl_target):
    ...
    exp_name = f"exp-fl-3_{this_file_name}"
    # loggerの設定をして設定情報を表示
    logger = set_exp_logging(exp_dir=save_dir, exp_name=exp_name)
    logger.info(f"ds_name: {ds_name}, fold_id: {k}, tgt_rank: {tgt_rank}, n_ratio: {n_ratio}, w_num: {w_num}, misclf_type: {misclf_type}, tgt_split: {tgt_split}, tgt_layer: {tgt_layer}")
    
    # localization結果をロード
    location_save_path = os.path.join(save_dir, f"exp-fl-3_location_n{n_ratio}_w{w_num}_{fl_target}.npy")
    proba_save_dir = os.path.join(save_dir, f"exp-fl-3_proba_n{n_ratio}_w{w_num}_{fl_target}")
