...
if __name__ == "__main__":
    ...
    # v-neuronの位置をロード
    vneuron_loc_dict = {}
    # corruption の正解/不正解データに対する v-neuron (tgt_ctがoriの場合はoriのv-neuronをロード)
    for cor_mis in ["cor", "mis"]:
        vneuron_loc_dict[cor_mis] = json.load(open(
            os.path.join(result_dir, f"vscore_l{start_li}tol{end_li}_all_label_{tgt_ct}_{used_column}_{cor_mis}.json")
            , "r"))["kn"]

    # 予測結果のメトリクスを格納するdataframe
    header = "dataset,operation," + ",".join(["ori"] + ct_list) # 列名
    ...
    for i, ct in enumerate(["ori"] + ct_list):
        ...
        if ct == "ori":
            ds = load_from_disk(os.path.join(dataset_dir, ori_ds_name))[used_column]
        ...
        for cor_mis in ["cor", "mis"]:
            # ニューロン値を変える対象の位置
            vn_pos = vneuron_loc_dict[cor_mis]

            # 2倍にするか0にするかの操作を繰り返す
            for op in ["enhance", "suppress"]:
                print(f"{'='*60}\ncor_mis: {cor_mis}, op: {op}\n{'='*60}")
                print("current acc_df")
                print(acc_df)
                
                # dfの前処理
                # "dataset" 列 が f"{tgt_ct}_{cor_mis}" であり, かつ, "operation" 列が op である行があるかどうかをチェック
                condition = (acc_df['dataset'] == f"{tgt_ct}_{cor_mis}") & (acc_df['operation'] == op)
                # 条件に合う行が存在しない場合
                if acc_df[condition].empty:
                    # acc, f1_dfの新しい行を作成
                    new_row_dict = {"dataset": f"{tgt_ct}_{cor_mis}", "operation": op}
                ...
                for data_idx, entry_dic in tqdm(enumerate(tgt_ds.iter(batch_size=batch_size)), 
                    ...
                    outputs = model(x, tgt_pos=tgt_pos, tgt_layer=start_li, imp_pos=vn_pos, imp_op=op)
