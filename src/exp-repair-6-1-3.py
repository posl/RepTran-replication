"""
exp-repair-6-1-3.py  —  evaluation for exp-repair-6 (p sensitivity)

Loads the patch produced by exp-repair-6-1-1.py and evaluates on repair/test set.
"""
import os, sys, json
import argparse
import numpy as np
...
if __name__ == "__main__":
    ...
    logger_obj = set_exp_logging(
        exp_dir=save_dir,
        exp_name=f"exp-repair-6-1-3_{setting_id}_reps{reps_id}_{tgt_split}",
    )
    logger_obj.info(f"ds={ds_name}, k={k}, tgt_rank={tgt_rank}, reps_id={reps_id}, "
                    f"p={p}, misclf_type={misclf_type}, fpfn={fpfn}, tgt_split={tgt_split}")
    ...
    print(f"===== Completed exp-repair-6-1-3.py (p={p}, reps={reps_id}, split={tgt_split}) =====")
