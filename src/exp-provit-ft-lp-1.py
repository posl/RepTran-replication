"""Runner: PRoViTFT+LP for a SINGLE repetition (reps_id) across all 9 fault
benchmarks of one dataset/fold.

Launched once per rep by exp-provit-ft-lp-2.py so each rep runs in a fresh
process (segfault isolation + full GPU release between reps). Results are saved
to a per-rep JSON **after every benchmark** and reloaded on restart (resume).

PRoViTFT+LP (PRoViT SAIV'24 §3.3), applied to the LAST encoder block's FFN:
  1. Fine-tune the last-block FFN (W_bef + W_aft) for ONE iteration.
  2. If repair-set efficacy == 100%, keep the FT'd model.
  3. Otherwise run PRoViTLP on the same block's W_aft to guarantee correctness
     on the (LayerNorm-linearised) repair set.
The FT step is stochastic (mini-batch shuffle seeded by reps_id); the LP step is
deterministic. RR/BR are measured on the true model.

Usage:
    python exp-provit-ft-lp-1.py c100 0 0          # ds, fold, reps_id
    python exp-provit-ft-lp-1.py tiny-imagenet 0 3
"""
import os, json, pickle
# Prevent transformers from importing TensorFlow (TF+torch on one GPU -> segfaults)
...
if __name__ == "__main__":
    ...
    logger.info(f"ds_name={ds_name}, k={k}, reps_id={reps_id}, lr={args.lr}, "
                f"gamma={args.gamma}, batch_size={args.batch_size}, eps={args.eps}")
    ...
    result_path = os.path.join(
        save_dir, f"results_lr{args.lr}_eps{args.eps}_rep{reps_id}.json")
    ...
    for misclf_type, fpfn, tgt_rank in BENCHMARKS:
        ...
        if eff_ft >= 1.0:
            ...
            used_lp = True
        logger.info(f"FT eff={eff_ft:.4f} (n_epochs={n_epochs}, ft_time={ft_time:.1f}s), "
                    f"used_lp={used_lp}, lp_status={lp_status}, solve={solve_time:.1f}s")
    ...
    # Print summary
    print(f"\nrep {reps_id}  {'Benchmark':<22} {'RR':>7} {'BR':>7} {'effF':>6} {'LP':>4} {'Ttot(s)':>9}")
    print("-" * 70)
    for r in all_results:
        ...
        print(f"{'':<6}{r['benchmark']:<22} {rr:>7} {br:>7} {ef:>6} {lp:>4} {r['Ttot']:>9.1f}")
