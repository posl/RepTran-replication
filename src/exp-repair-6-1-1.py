...
Localization is computed inline with the given p
  (p controls weight_fwd_imp=p, weight_grad_loss=1-p in calculate_top_n_flattened)

Fixed: N_w=236, alpha=10/11 (Arachne default), bounds=Arachne.

Usage:
    python exp-repair-6-1-1.py c100 0 1 0 --p 0.1 --misclf_type src_tgt
    python exp-repair-6-1-1.py c100 0 1 0 --p 0.9 --misclf_type tgt --fpfn fp
"""
import os, time, json
import argparse
import numpy as np
...
if __name__ == "__main__":
    ...
    # ── File paths ───────────────────────────────────────────────────────────
    patch_save_path       = os.path.join(save_dir, f"exp-repair-6-1-best_patch_{setting_id}_reps{reps_id}.npy")
    tgt_indices_save_path = os.path.join(save_dir, f"exp-repair-6-1-tgt_indices_{setting_id}_reps{reps_id}.npy")
    metrics_json_path     = os.path.join(save_dir, f"exp-repair-6-1-metrics_for_repair_{setting_id}_reps{reps_id}.json")
    tracker_save_path     = os.path.join(save_dir, f"exp-repair-6-1-tracker_{setting_id}_reps{reps_id}.pkl")

    # ── Logger ───────────────────────────────────────────────────────────────
    logger = set_exp_logging(exp_dir=save_dir, exp_name=f"exp-repair-6-1-1_{setting_id}_reps{reps_id}")
    logger.info(f"ds={ds_name}, k={k}, tgt_rank={tgt_rank}, reps_id={reps_id}, "
                f"p={p}, misclf_type={misclf_type}, fpfn={fpfn}")
    ...
    # Localization phase
    # ===============================================
    # p controls the weighting between ModFI and ModGL → must recompute per p value
    logger.info(f"Computing localization inline with p={p} "
                f"(weight_fwd_imp={p}, weight_grad_loss={1-p})")
    ...
    logger.info(f"Saved p-specific location to {loc_save_path}")
