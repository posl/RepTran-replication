...
def main(ds_name, k):
    ...
    # 中間ニューロン値のキャッシュのロード
    mid_cache_dir = os.path.join(exp_dir, f"cache_states_{tgt_split}")
    mid_save_path = os.path.join(mid_cache_dir, f"intermediate_states_l{tgt_layer}.npy")
    cached_mid_states = np.load(mid_save_path) # (tgt_splitのサンプル数(10000), 中間ニューロン数(3072))
    ...
    for data_idx, entry_dic in tqdm(enumerate(ds_preprocessed.iter(batch_size=1)), 
        ...
        tgt_mid = torch.unsqueeze(torch.tensor(cached_mid_states[data_idx]), 0).to(device) # (1, 3072)
