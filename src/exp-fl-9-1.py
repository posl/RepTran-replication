...
def main(ds_name, k, tgt_rank, misclf_type, fpfn, n):
    ...
    w_num = 8 * n * n
    identified_indices = calculate_top_n_flattened(grad_loss_list, fwd_imp_list, n=w_num, weight_grad_loss=1, weight_fwd_imp=0) # NOTE: 今回はロス勾配の順位で重みをソート
    ...
    location_save_path = os.path.join(location_save_dir, f"exp-fl-9_location_n{n}_weight_loss.npy")
