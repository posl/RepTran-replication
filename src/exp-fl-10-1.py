...
def main(ds_name, k, tgt_rank, misclf_type, fpfn, n):
    ...
    w_num = 8 * n * n
    identified_indices = calculate_top_n_flattened(grad_loss_list, fwd_imp_list, n=w_num, weight_grad_loss=0, weight_fwd_imp=1) # NOTE: 今回はfwd_impの順位で重みをソート
    ...
    location_save_path = os.path.join(location_save_dir, f"exp-fl-10_location_n{n}_weight_loss.npy")
...
if __name__ == "__main__":
    ...
    for k, tgt_rank, misclf_type, fpfn, n in product(k_list, tgt_rank_list, misclf_type_list, fpfn_list, n_list):
        print(f"\nStart: ds={ds}, k={k}, n={n}, tgt_rank={tgt_rank}, misclf_type={misclf_type}, fpfn={fpfn}\n====================================================================")
    ...
    result_df.to_csv(f"./exp-fl-10-1_time_n{n_str}.csv", index=False)