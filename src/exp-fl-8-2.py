...
def get_location_path(n, w_num, beta, fl_method, location_dir, generate_random=True):
    if fl_method == "ours":
        location_file = f"exp-fl-8_location_n{n}_beta{beta}_weight_ours.npy"
    elif fl_method == "bl":
...
def get_loss_diff_path(n, w_num, beta, fl_method, loss_diff_dir, op, cor_mis):
    ...
    if fl_method == "ours":
        loss_diff_file = f"exp-fl-8_loss_diff_n{n}_beta{beta}_{op}_{cor_mis}_weight_ours.npy"
    elif fl_method == "bl":
...
if __name__ == "__main__":
    ...
    output_csv_all = os.path.join(os.getcwd(), f"exp-fl-8-2_n{n_str}.csv")
