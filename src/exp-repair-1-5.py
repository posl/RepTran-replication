...
if __name__ == "__main__":
    ...
    # receive command line arguments ["repair" or "test"]
    tgt_split = sys.argv[1] if len(sys.argv) > 1 else "repair"
    assert tgt_split in ["repair", "test"], "Invalid tgt_split argument. Use 'repair' or 'test'."
    ...
    df_all = pd.DataFrame(big_records, columns=[
        "subject",
        "tgt. misclf. type", # [the type 
        "tgt_rank",          # of fault]
        "method",               # [
        "#modified weights",    # the repair 
        "alpha",                # setting
        "reps_id",              # ]
        "RR_tgt",       # repair outcome
        "BR_all",       # repair outcome
        "ΔACC.",        # repair outcome
        "TOT_TIME",     # repair outcome
    ])
    ...
    # 数値列を float に変換
    for col in ["reps_id", "RR_tgt", "BR_all", "ΔACC.", "TOT_TIME"]:
        df_all[col] = pd.to_numeric(df_all[col], errors='coerce')

    # ---------------------------
    # (B) (method, tgt. misclf. type, tgt_rank, #modified weights) ごとに平均
    # ---------------------------
    if tgt_split == "repair":
        df_grouped = (
