...
if __name__ == "__main__":
    ...
    for tgt_label in tgt_labels:
        result = subprocess.run(["python", "calc_integrated_gradient.py", ds_name, str(tgt_label)])
        if result.returncode != 0:
            exit(1)
        result = subprocess.run(["python", "identify_kn.py", ds_name, "--tgt_labels", str(tgt_label)])
        if result.returncode != 0:
            exit(1)
